...
import { FiArrowRight, FiCheck, FiAlertCircle, FiX } from "react-icons/fi";
...
const RegistrationForm = ({ selectedPlan, preferredDomain = "", institutionName = "", isExisting = false, isOpen, onClose }) => {
  ...
  // Sync preferred domain prop if it changes
  useEffect(() => {
    if (preferredDomain) {
      setFormData((prev) => ({ ...prev, preferredDomain }));
      handlePreferredDomainChange(preferredDomain);
    }
  }, [preferredDomain]);

  // Sync institution name prop if it changes
  useEffect(() => {
    if (institutionName) {
  ...
  useEffect(() => {
    if (isExisting && formData.preferredDomain) {
      setDomainStatus("available");
      setDomainCheckMessage("Verified Existing Domain (Renewal Mode)");
      return;
    }
      ...
      try {
        ...
        setDomainCheckMessage(
          domainsToCheck.length === 1
            ...
            : "No common domains are available for this name.",
        );
      } catch {
        if (isCancelled) return;
        setDomainStatus("idle");
        setDomainCheckMessage("Could not check domain availability.");
      }
    }, 600);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const planId = selectedPlan?.id || selectedPlan?._id || null;
      const payload = {
        ...
        selectedPlanPrice: Number(selectedPlan?.price || 0),
  ...
  if (isSubmitted) {
    return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-dark/60 backdrop-blur-sm animate-[fadeIn_0.3s_ease-out]">
        <div className="relative w-full max-w-lg rounded-3xl border border-white/30 bg-white p-8 sm:p-10 text-center shadow-2xl animate-[scaleUp_0.3s_ease-out]">
          <button onClick={onClose} className="absolute top-4 right-4 bg-red-50 text-red-500 hover:bg-red-500 hover:text-white rounded-full p-2 transition-colors duration-300">
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-dark/60 backdrop-blur-sm animate-[fadeIn_0.3s_ease-out]">
      <div className="relative w-full max-w-3xl max-h-[90vh] flex flex-col rounded-3xl border border-white/30 bg-white shadow-2xl overflow-hidden animate-[scaleUp_0.3s_ease-out]">
        {/* Header */}
        <div className="shrink-0 bg-linear-to-r from-secondary to-secondary-light px-6 py-4 flex items-center justify-between">
          ...
          <button 
            type="button" 
            onClick={onClose} 
            className="bg-red-500 hover:bg-red-600 text-white rounded-full p-2 transition-all duration-300 shadow-md hover:shadow-lg hover:-translate-y-0.5"
          >
            <FiX className="w-5 h-5" />
          </button>
        </div>
        ...
        <div>
          <label className="block text-xs font-bold text-dark/60 mb-1.5 uppercase tracking-wider">
            {isExisting ? "Your Domain (Existing)" : "Preferred Domain"} <span className="text-red-400">*</span>
          </label>
          <input
            id="preferred-domain"
            type="text"
            ...
            className={`${inputBase} ${errors.preferredDomain ? "border-red-400 ring-2 ring-red-400/20" : "border-dark/10"} ${isExisting ? "bg-dark/[0.03] text-dark/50 cursor-not-allowed border-dark/10" : ""}`}
