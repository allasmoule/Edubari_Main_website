...
const OrderSummary = ({ selectedPlan }) => {
    ...
    const oldPriceRaw = selectedPlan.oldPrice || selectedPlan.old_price;
    const originalPrice = oldPriceRaw
        ? (typeof oldPriceRaw === "string" ? parseInt(oldPriceRaw.replace(/[৳,]/g, "")) : Number(oldPriceRaw))
        : price;
            ...
            <div className="p-6">
                ...
                <div className="flex items-center justify-between pb-4 border-b border-dark/8">
                    <div>
                        ...
                        <p className="text-xs text-dark/50 mt-0.5">{selectedPlan.duration} {selectedPlan.duration_days ? `(${selectedPlan.duration_days} Days)` : ""}</p>
                    </div>
                    {selectedPlan.badge && (
