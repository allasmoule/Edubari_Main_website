...
const BlogCard = ({ post }) => {
  ...
  const coverImage = post.cover_image || post.image || "https://images.unsplash.com/photo-1516321318423-f06f85e504b3?q=80&w=600&auto=format&fit=crop";
  ...
  return (
    <Link
      href={`/blog/${postPath}`}
      className="group relative flex flex-col rounded-2xl border border-white/40 bg-white/50 backdrop-blur-sm overflow-hidden transition-all duration-300 hover:-translate-y-1.5 hover:shadow-xl hover:shadow-dark/8 hover:bg-white/80 hover:border-tertiary/20 cursor-pointer"
    >
      {/* Thumbnail */}
