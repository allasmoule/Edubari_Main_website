...
const Navigation = () => {
  ...
  const navItems = [
    ...
    { path: "/check-subscription", label: "Check Subscription", isButton: true },
  ];
  ...
  return (
    <nav className="sticky top-0 z-50 w-full bg-white/90 backdrop-blur-md py-4 px-4 sm:px-6 md:px-12 flex items-center justify-between border-b border-gray-100 shadow-sm transition-all duration-300">
      {/* Logo Section */}
      <Link
        href="/"
        onClick={(e) => {
        ...
        className="z-50 shrink-0 flex items-center cursor-pointer group select-none relative"
      >
      ...
      <div className="hidden lg:flex items-center px-6 py-2 rounded-full border border-gray-200 gap-6 bg-gray-50/50">
        {navItems.map((item) => {
          ...
          if (item.isButton) {
            return (
              <Link
                key={item.path}
                href={item.path}
                className="relative text-[14px] font-bold text-white bg-linear-to-r from-tertiary to-secondary px-5 py-2 rounded-full shadow-md hover:shadow-lg hover:shadow-tertiary/20 hover:-translate-y-0.5 active:translate-y-0 transition-all duration-300"
              >
                {item.label}
              </Link>
            );
      ...
      <div className="flex items-center gap-4 shrink-0 relative z-60">
        ...
        <div className="hidden lg:flex items-center gap-3">
          <Link
            href="https://software.edubari.bd/"
            target="_blank"
            rel="noopener noreferrer"
            className="relative font-bold shadow-md hover:shadow-lg group overflow-hidden rounded-full p-[2px] transition-all duration-300 hover:-translate-y-0.5 active:translate-y-0"
          >
      ...
      <div
        className={`fixed inset-0 bg-dark/40 backdrop-blur-sm z-40 lg:hidden transition-opacity duration-300 ${
      ...
      <div
        className={`fixed top-0 right-0 h-screen w-[85%] sm:w-[350px] bg-white shadow-2xl z-50 transform transition-transform duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] lg:hidden flex flex-col pt-24 pb-6 px-6 ${
          ...
          {navItems.map((item, index) => {
            ...
            return (
              <Link
                ...
                className={`p-4 rounded-xl text-[16px] transition-all duration-300 flex items-center ${
                  item.isButton
                    ? "bg-linear-to-r from-tertiary to-secondary text-white font-bold shadow-md justify-center mt-2"
                    : isActive
