    ...
    <aside className="space-y-6">
      ...
      <div className="rounded-2xl border border-white/40 bg-white/50 backdrop-blur-sm p-6 transition-all duration-300 hover:shadow-lg hover:shadow-dark/5">
        ...
        <div className="mt-4 space-y-4">
          ...
          {recentPosts.map((post) => {
            const postKey = post.id || post._id || post.slug || post.title;
            const postPath = post.slug || post.id || post._id || "";
            const coverImage = post.cover_image || post.image || "https://images.unsplash.com/photo-1516321318423-f06f85e504b3?q=80&w=150&auto=format&fit=crop";
            ...
            return (
              ...
              >
                <img
                  src={coverImage}
                  alt={post.title}
                  className="h-14 w-14 rounded-xl object-cover shrink-0 border border-white/30 group-hover:border-tertiary/20 transition-all duration-300 bg-gray-100"
                  loading="lazy"
                />
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs text-dark/40">{postDate}</p>
                </div>
              </Link>
            );
          })}
        ...
        <div className="mt-4 space-y-1.5">
          {categoryCounts.map((cat) => (
            <button
              ...
              className="w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-sm font-medium text-dark/70 hover:bg-tertiary/8 hover:text-tertiary transition-all duration-200 group cursor-pointer"
            >
              <span>{cat.name}</span>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {popularTags.map((tag) => (
            <button
              ...
              className="px-3 py-1.5 rounded-full bg-dark/5 text-xs font-semibold text-dark/60 hover:bg-tertiary/10 hover:text-tertiary border border-transparent hover:border-tertiary/20 transition-all duration-200 cursor-pointer"
            >
              {tag}
            </button>
          ))}
      ...
      <div className="rounded-2xl border border-tertiary/20 bg-linear-to-br from-tertiary/8 to-[#8B5CF6]/8 backdrop-blur-sm p-6 transition-all duration-300 hover:shadow-lg hover:shadow-tertiary/10">
        ...
        <form className="mt-4 space-y-2.5" onSubmit={handleSubscribe}>
          ...
          <button
            type="submit"
            className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-linear-to-r from-tertiary to-[#8B5CF6] text-white text-sm font-bold shadow-md shadow-tertiary/25 hover:shadow-lg hover:shadow-tertiary/35 hover:-translate-y-0.5 active:translate-y-0 transition-all duration-300 cursor-pointer"
          >
            Subscribe
